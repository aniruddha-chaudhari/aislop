/**
 * Lightweight emotion label from spoken dialogue text (no extra LLM call).
 * Values must exist in `EMOTION_VARIANTS` for Peter / Narrator in characterImages.ts.
 */
export function inferDialogueEmotion(text: string): string {
  ...
  if (
    /\b(hate|hates|hated|angry|anger|furious|rage|raging|annoyed|idiot|stupid|damn it|screw you|shut up)\b/i.test(
      t
    )
  ) {
    return 'angry';
  ...
  if (
    /\b(sad|sadly|cry|crying|tears|sob|sorry|depressed|mourn|tragic|unfortunate|heartbreaking)\b/i.test(t)
  ) {
    return 'sad';
  }
  if (
